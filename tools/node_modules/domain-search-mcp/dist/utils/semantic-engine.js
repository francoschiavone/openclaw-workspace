...
exports.SYNONYMS = exports.INDUSTRY_TERMS = exports.MODERN_PREFIXES = exports.MODERN_SUFFIXES = void 0;
exports.segmentWords = segmentWords;
exports.getSynonyms = getSynonyms;
exports.getIndustryTerms = getIndustryTerms;
...
exports.scoreDomainName = scoreDomainName;
exports.filterKnownTakenSuggestions = filterKnownTakenSuggestions;
...
exports.INDUSTRY_TERMS = INDUSTRY_TERMS;
/**
...
exports.MODERN_SUFFIXES = MODERN_SUFFIXES;
/**
...
exports.MODERN_PREFIXES = MODERN_PREFIXES;
/**
...
const INDUSTRY_TERM_SET = new Set(Object.values(INDUSTRY_TERMS).flat());
function filterSuggestionWords(words) {
    ...
    for (const word of words) {
        ...
        const isMeaningful = COMMON_WORDS.has(normalized) ||
            Object.prototype.hasOwnProperty.call(SYNONYMS, normalized) ||
            INDUSTRY_TERM_SET.has(normalized);
        if (!isShortAllowed && normalized.length < 3)
            continue;
        if (!isMeaningful && normalized.length < 4)
            continue;
...
function generateSmartSuggestions(input, options = {}) {
    const { maxSuggestions = 50, includePortmanteau = true, includeSynonyms = true, includeIndustryTerms = true, industry, } = options;
 ...
 */
function scoreDomainName(name, originalInput) {
    let score = 50;
 ...
 */
async function filterKnownTakenSuggestions(suggestions, tld) {
    if (!config_js_1.config.negativeCache.enabled || suggestions.length === 0) {
        return suggestions;
    }
    // Build FQDNs and query the negative cache
    const fqdns = suggestions.map(s => `${s}.${tld}`);
    const result = await (0, negative_cache_js_1.queryTakenDomains)(fqdns);
