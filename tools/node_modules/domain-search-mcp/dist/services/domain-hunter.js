...
async function huntDomains(criteria) {
    ...
    if (config_js_1.config.negativeCache.enabled && toCheck.length > 0) {
        const fqdns = toCheck.flatMap(({ name }) => tlds.map(tld => `${name}.${tld}`));
        try {
            const takenResult = await (0, negative_cache_js_1.queryTakenDomains)(fqdns);
            if (takenResult.taken.size > 0) {
                const beforeCount = toCheck.length;
                // Keep candidates where at least one TLD is not known-taken
                toCheck = toCheck.filter(({ name }) => !tlds.every(tld => takenResult.taken.has(`${name}.${tld}`.toLowerCase())));
                const filtered = beforeCount - toCheck.length;
                if (filtered > 0) {
                    insights.push(`Pre-filtered ${filtered} known-taken domains`);
                    logger_js_1.logger.debug('Domain hunter pre-filter', {
                        before: beforeCount,
            ...
            logger_js_1.logger.debug('Negative cache query failed, continuing without pre-filter', {
                error: error instanceof Error ? error.message : String(error),
            });
        }
    }
